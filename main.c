...
void button_intr_callback(uint8_t gpio) {
    ...
    if (((now - last_button_event_time) > DEBOUNCE_TIME) && (gpio_read(gpio) == 1)) {
        if (((now - last_reset_event_time) > RESET_TIME) && (gpio == MAIN_BUTTON_GPIO)) {
            xTaskCreate(reset_task, "Reset", 256, NULL, 2, NULL);
        } else {
...
void thermostat_init() {
    ...
    gpio_set_pullup(DOWNTEMP_BUTTON_GPIO, false, false);
...
homekit_characteristic_t serial = HOMEKIT_CHARACTERISTIC_(SERIAL_NUMBER, "ESP N/A");
...
void create_accessory_name() {
    ...
    snprintf(name_value, 14, "ESP %02X%02X%02X", macaddr[3], macaddr[4], macaddr[5]);
